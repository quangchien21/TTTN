#define BLYNK_TEMPLATE_ID "TMPL6d1iMKiVJ"
#define BLYNK_TEMPLATE_NAME "Gas Leaks"
#define BLYNK_AUTH_TOKEN "12INakmMyWnk0uGp0MLyF8-TT3lrpQ9S"
...
//#define GPS_RX 12  // D6 - nối chân TX của gps
//#define GPS_TX 14  // D5 - nối chân RX của gps

#define GPS_RX 15  // D8 - nối chân TX của gps
#define GPS_TX 13  // D7 - nối chân RX của gps
SoftwareSerial gpsSerial(GPS_RX, GPS_TX);
TinyGPSPlus gps;

// 🔹 Biến trạng thái
char trangThai = '4'; // Mặc định là DỪNG
bool leakDetected = false; // Biến để kiểm tra trạng thái phát hiện vị trí rò rỉ
BlynkTimer timer;
...
void sendGPSToFirebase() {
    // Nếu đã phát hiện vị trí rò rỉ, dừng gửi tọa độ lên Firebase
    if (leakDetected) {
        Serial.println("🚨 Đã phát hiện vị trí rò rỉ. Dừng cập nhật tọa độ lên Firebase!");
        return;
    }
    ...
    if (gps.location.isValid()) {
        ...
        sendToFirebase(20.836801, 106.694364, "15/04/2025 09:38:50");
    }
}
...
void handleSerialInput() {
    if (espSerial.available()) {
        ...
        if (data.startsWith("Gas:")) {
            ...
            Blynk.virtualWrite(V4, giatri_gas);
            ...
            if (giatri_gas >= 500) {
                  ...
                  leakDetected = true;  // Đánh dấu đã phát hiện rò rỉ

                  // Gửi GPS lên Blynk nếu phát hiện khí gas
                  sendGPSToBlynk();
            } else {
                  if(giatri_gas < 500 && giatri_gas >= 300) {
...
BLYNK_WRITE(V0) { 
    int value = param.asInt(); 
    Serial.println("Giá trị nhận được từ V0: " + String(value)); 
    guiLenh(value == 1 ? '7' : '4'); 
}
BLYNK_WRITE(V1) { 
    int value = param.asInt(); 
